...
digits:
    ...
    ldi r1, 10   # counter
1:
    lcdcr r2     # read busy flag and DDRAM address (BF A6 A5 A4 A3 A2 A1 A0)
    addi r2, 2   # add 2 to address / move cursor forward by 2
    ori r2, 0x80 # set top bit
    lcdcw r2     # set DDRAM address (1 A6 A5 A4 A3 A2 A1 A0)
...
modify:
    # Modify "e".
    ldi r0, 0x81  # set DDRAM address to 1 ("e")
    lcdcw r0
    lcddr r1      # read display data ("e"), moves to next character
    lcdcw r0      # reposition on "e"
    addi r1, 16   # translate ASCII "e" into "u"
    lcddw r1      # write "u"
